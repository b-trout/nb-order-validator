...
from src.nb_order_validator.main import get_execution_counts, is_consecutive, main
...
@pytest.mark.parametrize("counts, expected", [
    ...
    ([None], False),            # Invalid: Only unexecuted cell (treated as non-consecutive)
])
...
def test_get_execution_counts_valid(tmp_path: Path) -> None:
    ...
    counts = get_execution_counts(str(f))
    assert counts == [1, 2, 3]
...
def test_get_execution_counts_file_not_found() -> None:
    ...
    """
    counts = get_execution_counts("ghost_file.ipynb")
    assert counts == []
...
def test_main_multiple_files(tmp_path: Path, capsys: CaptureFixture[str]) -> None:
    """
